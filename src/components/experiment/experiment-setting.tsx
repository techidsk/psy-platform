...
import { exprimentSettingSchema } from "@/lib/validations/auth"
...
export function ExperimentSetting() {
    ...
    return (
        <div>
            <button className="btn btn-outline rounded flex gap-2 items-center" onClick={handleToggle}>
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div className="grid gap-2">
                        <div className="grid gap-1">
                            <label className="sr-only" htmlFor="display-num">图片显示数量</label>
                            <input
                                id="display-num"
                            ...
                            />
                            {errors?.display_num && (
                                <p className="px-1 text-xs text-red-600">{errors.display_num.message}</p>
                            )}
                        </div>
