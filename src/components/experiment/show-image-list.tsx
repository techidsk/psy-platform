...
export function ShowImageList({ experimentList }: ImageListProps) {
    ...
    const totalLength: number =
        experimentList.length > 1
            ? (experimentList[experimentList.length - 1]?.timestamp || 0) -
              (experimentList[0]?.timestamp || 0) +
              1
            : 1;
    ...
    useEffect(() => {
        let temp: ImageResponse[] = experimentList
            .filter((item) => (item.timestamp || 0) < seconds)
    ...
    function setSliceList(newIndex: number) {
        ...
        setList(
            tmp.slice(s, e).map((item: ImageResponse, idx: number) => ({ ...item, idx: idx + s }))
        );
    ...
    return (
        <div className="flex gap-4 w-full flex-col">
            ...
            <div className="flex flex-col gap-4 justify-center items-center">
                <h1 className="text-xl text-center select-none">
                    当前时间戳： {` `}
                    {formatTime(seconds)}
                    {` / `}
                ...
                <div className="flex gap-4 justify-center items-center">
                    <button
                        className={`btn btn-sm rounded ${
                            pause ? 'btn-primary' : 'btn-ghost btn-outline border-solid'
                        }`}
