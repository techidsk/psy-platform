 ...
 * 执行实验状态清理任务：
 * 1. 标记超时的实验为 FAILED
 * 2. 标记所有 trail 都失败的实验为 FAILED
 * 3. 清理长时间处于 GENERATING 状态的 trail
 */
export async function runExperimentJobs() {
    try {
        ...
        SELECT id
            FROM (
                SELECT ue.id,
                    ...
                    TIMESTAMPDIFF(MINUTE, ue.start_time, NOW()) AS minutes_since_start
        ...
        logger.error('执行实验状态清理任务失败:', error);
        throw error;
    }
}
