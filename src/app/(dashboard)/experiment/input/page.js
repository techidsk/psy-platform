...
export default function Input() {
    ...
    function submit(e) {
        if (text.length < MIN_CHAR) {
            setOpen(true)
            setMessage({ title: '字数未满足要求', msg: `请至少输入${MIN_CHAR}个字符，当前已输入${text.length}个字符` })
            setTimeout(() => {
                setOpen(false);
            }, 5000);
            return
                ...
                <div className='flex flex-col justify-center items-center gap-8 w-full'>
                    ...
                    <div className='flex flex-col gap-4 w-full'>
                        <div className='flex gap-4 flex-row-reverse w-full'>
                            <button className={loading ? 'btn btn-primary loading w-48' : 'btn btn-primary w-48'} disabled={loading} onClick={submit}>发送</button>
                            <label htmlFor="settings" className="cursor-pointer flex justify-center items-center"><MixerHorizontalIcon /></label>
                            ...
                            <div className="modal">
                                <div className="modal-box">
                                    ...
                                    <div className="modal-action">
                                        <label htmlFor="settings" className="btn btn-primary btn-sm" onClick={saveSettings}>保存</label>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className='flex gap-4 flex-row-reverse w-full'>
                            <label htmlFor="my-modal" className="btn btn-outline btn-secondary w-48">完成测验</label>
                            {/* Put this part before </body> tag */}
                            <input type="checkbox" id="my-modal" className="modal-toggle" />
                            <div className="modal">
                                <div className="modal-box">
                                    <h3 className="font-bold text-lg">确认提交</h3>
                                    <p className="py-4">您是否确定提交测验？提交后无法再次编辑。</p>
                                    <div className="modal-action">
                                        <div className="flex gap-4 justify-end">
                                            <label htmlFor="my-modal" className="btn btn-outline btn-secondary">继续编辑</label>
